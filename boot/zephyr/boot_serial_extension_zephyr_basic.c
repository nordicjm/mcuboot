...
static int bs_custom_img_list(const struct nmgr_hdr *hdr, const char *buffer,
                              int len, zcbor_state_t *cs)
{
    ...
    if (hdr->nh_group != ZEPHYR_MGMT_GRP_BASIC || hdr->nh_op != NMGR_OP_READ ||
        hdr->nh_id != ZEPHYR_MGMT_GRP_BASIC_CMD_IMAGE_LIST) {
        return MGMT_ERR_ENOTSUP;
    }
...
MCUMGR_HANDLER_DEFINE(image_list, bs_custom_img_list);
